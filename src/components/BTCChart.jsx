...
export default function BTCChart() {
  ...
  const [activeCoin, setActiveCoin] = useState({ symbol: 'BTCUSDT', label: 'BTC', name: 'Bitcoin', color: '#f7931a' });
  ...
  const fetchPrices = () => {
    ...
    const iv = ivMap[activeInterval];
    const limit = limitMap[activeInterval];
    fetch(`https://api.binance.com/api/v3/klines?symbol=${activeCoin.symbol}&interval=${iv}&limit=${limit}`)
      .then(r => r.json())
      .then(data => {
  ...
  useEffect(() => {
    ...
    ctx.lineWidth = 1;
    for (let i = 0; i <= 3; i++) {
      const yPos = pad.top + (i / 3) * (H - pad.top - pad.bottom);
      ctx.beginPath();
            ...
            <div style={{ position: 'absolute', top: '22px', left: 0, background: '#1e2538', border: '1px solid rgba(99,102,241,0.5)', zIndex: 100, minWidth: '130px' }}>
              {COINS.map(coin => (
                <button key={coin.symbol} onClick={() => { setActiveCoin(coin); setShowCoinPicker(false); }}
                  style={{ display: 'flex', alignItems: 'center', gap: '6px', width: '100%', padding: '5px 8px', background: activeCoin.symbol === coin.symbol ? 'rgba(99,102,241,0.2)' : 'transparent', border: 'none', color: 'white', cursor: 'pointer', fontSize: '8px', textAlign: 'left', flexDirection: 'row' }}>
