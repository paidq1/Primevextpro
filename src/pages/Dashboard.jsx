...
export default function Dashboard() {
  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh', background: '#0e1628', display: 'flex', fontFamily: "'Segoe UI', sans-serif" }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          ...
          {/* Top Nav */}
          <div style={{ background: '#132035', padding: '6px 8px', display: 'flex', alignItems: 'center', gap: '4px', flexShrink: 0, borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <div style={{ display: 'flex', gap: '3px', flexShrink: 0 }}>
              ...
              <button onClick={() => navigate('/dashboard/kyc')} style={{ padding: '4px 10px', background: u.kycStatus === 'approved' ? 'rgba(34,197,94,0.2)' : 'rgba(239,68,68,0.15)', border: u.kycStatus === 'approved' ? '1px solid #22c55e' : '1px solid #f59e0b', color: u.kycStatus === 'approved' ? '#22c55e' : '#f59e0b', fontSize: '8px', fontWeight: '700', cursor: 'pointer', animation: (!u.kycStatus || u.kycStatus === 'pending') ? 'pulse 2s infinite' : 'none' }}>KYC {u.kycStatus === 'approved' ? '✓' : '!'}</button>
            </div>
            <div style={{ marginLeft: 'auto', display: 'flex', gap: '3px', alignItems: 'center' }}>
              ...
              <div onClick={() => navigate('/dashboard/profile')} style={{ width: '32px', height: '32px', borderRadius: '50%', background: '#6366f1', cursor: 'pointer', overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {u.avatar ? <img src={u.avatar} style={{ width: '32px', height: '32px', objectFit: 'cover' }} /> : <User size={13} color="white" />}
              </div>
            </div>
          </div>
          ...
          <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '8px', minWidth: 0 }}>
              ...
              {/* Chart */}
              <div style={{ marginBottom: '12px', border: '1px solid rgba(99,102,241,0.5)', background: '#1a2e4a', height: '240px', position: 'relative', overflow: 'hidden' }}>
                <iframe
                  src="https://s.tradingview.com/widgetembed/?symbol=BINANCE%3ABTCUSDT&interval=D&hidesidetoolbar=1&symboledit=0&saveimage=0&toolbarbg=131722&studies=%5B%5D&theme=dark&style=3&timezone=Etc%2FUTC&withdateranges=1&showpopupbutton=0&locale=en&hide_top_toolbar=0&hide_legend=0&hide_volume=0&scale=1&bgcolor=%23131722&gridcolor=rgba(255%2C255%2C255%2C0.04)"
                  style={{ position: 'absolute', top: 0, left: 0, width: '160%', height: '150%', border: 'none', transform: 'scale(0.65)', transformOrigin: 'top left' }}
            ...
            {/* Right Panel */}
            <div style={{ width: '155px', padding: '10px', overflowY: 'auto', flexShrink: 0, paddingTop: '34px' }}>
              <div style={{ background: '#1a2e4a', border: '1px solid rgba(99,102,241,0.4)', padding: '10px' }}>
