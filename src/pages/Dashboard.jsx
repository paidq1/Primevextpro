      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ background: '#141824', padding: '8px 14px', display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0, borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: '6px', alignItems: 'center' }}>
            ...
            <button onClick={() => navigate('/dashboard/profile')} style={{ width: '28px', height: '28px', borderRadius: '50%', background: '#6366f1', border: 'none', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', padding: 0 }}>{u.avatar ? <img src={'https://primevextpro.onrender.com' + u.avatar} style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={e => { e.target.style.display='none'; e.target.nextSibling.style.display='flex'; }} /> : null}<span style={{ display: u.avatar ? 'none' : 'flex', fontSize: '9px', fontWeight: '700' }}>{(u.firstName?.[0] || '') + (u.lastName?.[0] || '')}</span></button>
          </div>
        </div>

        {/* Admin Message */}
        {u.adminMessage && (
          <div style={{ background: 'rgba(245,158,11,0.15)', border: '1px solid #f59e0b', padding: '8px 14px', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '8px', color: '#f59e0b' }}>
            ...
            <span><strong>Notice:</strong> {u.adminMessage}</span>
          </div>
        )}
