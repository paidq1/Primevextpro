      ...
      {showSuccess && (
        <>
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', zIndex: 151, background: 'white', padding: '28px 20px', width: '260px', textAlign: 'center', borderRadius: '4px' }}>
            ...
            <div style={{ color: '#111', fontSize: '14px', fontWeight: '700', marginBottom: '8px' }}>Bot Activated!</div>
            <div style={{ color: '#555', fontSize: '9px', marginBottom: '20px', lineHeight: '1.6' }}>Your bot has been activated successfully.</div>
          ...
          {bots.map((bot, i) => {
            ...
            return (
              <div key={i} style={{ background: '#1a2e4a', border: '1px solid rgba(99,102,241,0.3)', padding: '12px', position: 'relative' }}>
                ...
                <div style={{ color: bot.color, fontSize: '13px', fontWeight: '800', marginBottom: '10px' }}>${bot.amount.toLocaleString()}</div>
                {[
