...
const TRADERS_DEFAULT = [
  ...
  { id: 4, name: 'Nicola Duke', bio: 'A UK-based forex trader and educator specializing in technical analysis and price action trading. Nicola is known for her clear and structured approach to trading the forex markets, particularly GBPUSD and EURUSD pairs. She combines classical chart patterns with modern technical indicators to identify high-probability trade setups.', location: 'United Kingdom', flag: '🇬🇧', followers: '1.6k', risk: 5.2, favorite: 'GBPUSD', totalTrades: 540, totalLoss: 15, profitShare: 19.5, winRate: 81, img: 'https://ui-avatars.com/api/?name=Nicola+Duke&background=f59e0b&color=fff&size=128', verified: true },
  { id: 5, name: 'Anton Kreil', bio: 'A former Goldman Sachs trader and founder of the Institute of Trading and Portfolio Management (ITPM). Anton has over 20 years of professional trading experience and has trained thousands of traders worldwide. His approach is rooted in institutional trading principles, risk management, and portfolio diversification across multiple asset classes.', location: 'London, UK', flag: '🇬🇧', followers: '2.8k', risk: 7.1, favorite: 'ETH', totalTrades: 1200, totalLoss: 45, profitShare: 12.5, winRate: 88, img: 'https://ui-avatars.com/api/?name=Anton+Kreil&background=3b82f6&color=fff&size=128', verified: true },
  ...
  { id: 7, name: 'Nial Fuller', bio: 'An Australian forex trader and educator widely regarded as one of the leading authorities on price action trading. Nial founded Learn To Trade The Market, one of the most popular forex trading education websites. His trading philosophy centers on simplicity, patience, and mastering price action setups on higher timeframes.', location: 'Australia', flag: '🇦🇺', followers: '1.8k', risk: 5.1, favorite: 'GBPUSD', totalTrades: 610, totalLoss: 18, profitShare: 22.3, winRate: 84, img: 'https://ui-avatars.com/api/?name=Nial+Fuller&background=8b5cf6&color=fff&size=128', verified: true },
  { id: 8, name: 'Anne-Marie Baiynd', bio: 'A seasoned trader, author, and market analyst based in Texas. Anne-Marie is the author of The Trading Book and is known for her expertise in technical analysis and options trading. She combines market internals, price action, and options flow to identify high-probability trading opportunities in US equities and indices.', location: 'Texas, USA', flag: '🇺🇸', followers: '1.4k', risk: 4.9, favorite: 'SPX', totalTrades: 720, totalLoss: 22, profitShare: 17.8, winRate: 80, img: 'https://ui-avatars.com/api/?name=Anne-Marie+Baiynd&background=06b6d4&color=fff&size=128', verified: true },
];
  ...
  useEffect(() => {
    getTraders().then(data => {
      if (Array.isArray(data) && data.length > 0) {
        // Merge API traders with default bios if missing
        const merged = data.map(t => {
          const def = TRADERS_DEFAULT.find(d => d.name === t.name);
          return { ...t, bio: t.bio || (def ? def.bio : '') };
        });
        setTraders(merged);
      }
      setLoading(false);
